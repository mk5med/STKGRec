...
def pad_batch_of_lists_masks(
    batch_of_lists, sequence_tim_batch, tim_dis_gap_batch, max_len
):
    ...
    padded_mask = [
        [1.0] * (len(l) - 1) + [0.0] * (max_len - len(l) + 1) for l in batch_of_lists
    ]
    padde_mask_non_local = [
        [1.0] * (len(l)) + [0.0] * (max_len - len(l)) for l in batch_of_lists
    ]
...
def pad_batch_of_lists_masks_test(batch_of_lists, max_len):
    ...
    padded_mask = [
        [0.0] * (len(l) - 2) + [1.0] + [0.0] * (max_len - len(l) + 1)
        for l in batch_of_lists
    ]
    padde_mask_non_local = [
        [1.0] * (len(l) - 1) + [0.0] * (max_len - len(l) + 1) for l in batch_of_lists
    ]
...
def caculate_poi_distance_time(args, poi_coors, trans, temp_max, distance_max):
    ...
    for poi_gap in trans:
        ...
        for id in trans[poi_gap]:
            time_between += id[0]
        tem_dis_score[poi_gap] = (
            (time_between / len(trans[poi_gap])) / temp_max
        ) * args["td"] + (id[1] / distance_max) * (1 - args["td"])
...
def generate_detailed_batch_data(
    n_locs,
    one_train_batch,
    data_neural,
    tim_dis_dict,
    ...
    for sample in one_train_batch:
        ...
        session_sequence_current = [
            s[0] for s in data_neural[sample[0]]["sessions"][sample[1]]
        ]
        session_sequence_tim_current = [
            s[1] for s in data_neural[sample[0]]["sessions"][sample[1]]
        ]
        session_sequence_tim_dis = [
            int(tim_dis_dict[tuple(s[1])] + n_tim_rel)
            for s in data_neural[sample[0]]["sessions_trans"][sample[1]]
        ]
        session_sequence_dilated_input = create_dilated_rnn_input(
            session_sequence_current, poi_temporal_distance_matrix
        )
...
def construct_data(kg_upt_data, kg_ptp_data, tim_dis_dict, n_tim_rel):
    ...
    rel_ptp = [
        int(tim_dis_dict[tuple(triple[1])] + n_tim_rel) for triple in kg_ptp_data
    ]
