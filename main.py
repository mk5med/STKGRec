...
def evaluate(network, mode, batch_size=64):
    ...
    data_test, test_idx = generate_input_long_history(
        data_neural, mode, candidate=candidate
    )
    users_acc = {}
    with torch.no_grad():
        tim_dis_dict = ptp_dict
...
def get_params():
    ...
    parser.add_argument("--data", type=str, default="nyc", help="data name")
    ...
    parser.add_argument(
        "--hidden_size", type=int, default=100, metavar="N", help="hidden layer size"
    )
    parser.add_argument(
        "--lr", type=float, default=0.0001, metavar="LR", help="learning rate"
    )
    parser.add_argument(
        "--wd", type=float, default=1 * 1e-6, metavar="LR", help="weight_decay"
    )
    parser.add_argument(
        "--epochs",
        type=int,
    ...
    parser.add_argument(
        "--td", type=float, default=0.4, help="Lambda between temporal and distance."
    )
