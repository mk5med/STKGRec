...
class DataFoursquare(object):
    ...
    def filter_users_by_length(self):  # self.data:[uid]={[loc,time]}
        ...
        ]  # 保留用户访问的序列大于trace_len_min的用户
        pick3 = sorted(
            [(x, len(self.data[x])) for x in uid_3], key=lambda x: x[1], reverse=True
        )  # (user, user_len)
        pid_3 = [
            x for x in self.venues if self.venues[x] >= self.location_global_visit_min
        ]  # 保留被用户访问大于loc的最小值的loc
        pid_pic3 = sorted(
            [(x, self.venues[x]) for x in pid_3], key=lambda x: x[1], reverse=True
        )  # (loc,loc_len)
        ...
        for u in pick3:  # 开始为每个用户划分session
            ...
            topk = Counter(
                [x[0] for x in info]
            ).most_common()  # 列表储存每一个用户访问loc的频次
            topk1 = [
                x[0] for x in topk if x[1] > 1
            ]  # 储存loc,过滤一个用户访问至少访问同一个地点两次
            sessions = {}
            for i, record in enumerate(info):  # record储存[loc,tim]
                ...
                if poi not in pid_3 and poi not in topk1:  # 过滤掉被访问频次低的loc
                    ...
                    continue
                if (
                    i == 0 or len(sessions) == 0
                ):  # session[user]={[loc1,tim1],[loc2,tim2]}
                    sessions[sid] = [record]
                else:
                    if (tid - last_tid) / 3600 > self.hour_gap or len(
                        sessions[sid - 1]
                    ) > self.session_max:  # 大于24h或者每一个session的长度大于10
                        sessions[sid] = [record]
        ...
        for u in self.user_filter3:  # 获取用户
            ...
            if u not in self.uid_list:
                self.uid_list[u] = [
                    len(self.uid_list),
                    len(sessions),
                ]  # 存储user字典，[user:[len(user_id),len(user_session)]]
        ...
        for u in tqdm(self.uid_list):
            ...
            for sid in sessions:  # step1: 获取用户的session
                ...
                for i in range(
                    ...
                    ti_pre = int(
                        time.mktime(
                            time.strptime(sessions[sid][i][1], "%Y-%m-%d %H:%M:%S")
                        )
                    )
                    ...
                    ti_next = int(
                        time.mktime(
                            time.strptime(sessions[sid][j][1], "%Y-%m-%d %H:%M:%S")
                        )
                    )
                ...
                sessions_utp[sid] = [
                    [uid, self.tid_list_48(p[1]), self.vid_list[p[0]][0]]
                    for p in sessions[sid]
                ]  # 映射loc->loc_id
            ...
            self.data_temp[self.uid_list[u][0]] = {
                "sessions_utp": sessions_utp,
                "sessions_id": sessions_id,
            }

    def construct_data(self, n_tim_rel, tim_dis_dict):
        train_kg_ptp, train_kg_upt, train_kg = [], [], []
        train_kg_dict, train_kg = collections.defaultdict(
            list
        ...
        rel_ptp = [
            int(tim_dis_dict[tuple(triple[1])] + n_tim_rel) for triple in self.train_ptp
        ]
    ...
    def prepare_kg_data(self):
        for u in self.data_neural:
            for k in self.data_neural[u]["train"]:
                ...
                self.train_ptp.extend(
                    [q for q in self.data_neural[u]["sessions_trans"][k]]
                )
    ...
    def save_variables(self):
        ...
        pickle.dump(
            foursquare_dataset, open(self.SAVE_PATH + self.save_name + ".pkl", "wb")
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--trace_min", type=int, default=10, help="raw trace length filter threshold"
    )
    parser.add_argument(
        "--global_visit", type=int, default=10, help="location global visit threshold"
    )
    parser.add_argument(
        "--hour_gap",
        type=int,
...
if __name__ == "__main__":
    ...
    data_generator = DataFoursquare(
        ...
        time_split=args.time_gap,
        distance_split=args.distance_gap,
    )
