...
class STKGRec(nn.Module):
    def __init__(
        ...
        self.relations = nn.Embedding(
            self.n_tim_rel + self.n_tim_dis_gap, self.emb_size
        )
        self.gru = nn.GRU(input_size=self.emb_size, hidden_size=self.hidden_units)
        self.gru_history = nn.GRU(
            input_size=self.emb_size, hidden_size=self.hidden_units
        )
        self.rel_rnn = nn.GRU(
            input_size=self.emb_size, hidden_size=self.hidden_units
        )  # could be the same as self.gru
        self.dilated_rnn = nn.GRUCell(
            input_size=self.emb_size, hidden_size=self.hidden_units
        )
        ...
        self.W_R = nn.Parameter(
            torch.Tensor(
                self.n_tim_rel + self.n_tim_dis_gap, self.emb_size, self.emb_size
            )
        )
    ...
    def sessions_score(
        ...
        for ii in range(batch_size):
            current_session_input_dilated_rnn_index = sequence_dilated_rnn_index_batch[
                ii
            ]
            ...
            for index_dilated in range(len(current_session_input_dilated_rnn_index)):
                ...
                if index_dilated == 0:
                    ...
                    h = self.dilated_rnn(
                        hidden_current, dilated_gru_outs_h[index_dilated_explicit]
                    )
                    dilated_gru_outs_h.append(h)
            dilated_gru_outs_h.append(
            ...
            if is_train:
                ...
                for iii in range(sequence_length - 1):
                    current_session_represent_rep_item = current_session_embed[
                        0 : iii + 1
                    ]
                    current_session_represent_rep_item = torch.sum(
                        current_session_represent_rep_item, dim=0
                    ).unsqueeze(0) / (iii + 1)
                    current_session_represent_list.append(
                        current_session_represent_rep_item
                    )
                    current_relation = current_tim_dis_rel[0 : iii + 1]
                    current_relation = torch.sum(current_relation, dim=0).unsqueeze(0)
                    current_relation_list.append(current_relation)
            ...
            current_rel_att = F.softmax(
                torch.mm(current, current_rel_present.transpose(0, 1))
            )
            current = torch.mm(current_rel_att, current)
            out_rel_current_padd = Variable(
                torch.FloatTensor(
                    sequence_size - sequence_length + 1, self.emb_size
                ).zero_(),
                requires_grad=False,
            ).cuda()
            ...
            for jj in range(session_id_current):
                ...
                trans_rel = []
                sequence_loc = [
                    s[0] for s in self.data_neural[user_id_current]["sessions"][jj]
                ]
                sequence_len = len(sequence_loc)
                sequence_tim = [
                    s[1] for s in self.data_neural[user_id_current]["sessions"][jj]
                ]
                sequence_tim_dis_gap = [
                    int(self.tim_dis_dict[tuple(s[1])] + self.n_tim_rel)
                    for s in self.data_neural[user_id_current]["sessions_trans"][jj]
                ]
                ...
                sequence_tim_dis_gap = Variable(
                    torch.LongTensor(np.array(sequence_tim_dis_gap))
                ).cuda()
                ...
                rel_padd = Variable(
                    torch.FloatTensor(1, self.emb_size).zero_(), requires_grad=False
                ).cuda()
                sequence_tim_dis = torch.cat(
                    [sequence_tim_dis_gap[: sequence_len - 1], rel_padd], dim=0
                )
                trans_vec = sequence_loc + sequence_tim_dis
                sequence_user = user_id_vec.unsqueeze(0).expand(
                    sequence_len, self.emb_size
                )
                user_loc_tim_vec = (
                    sequence_user + sequence_loc + sequence_tim + sequence_tim_dis
                )
                att_seq = F.softmax(
                    torch.mm(trans_vec, user_loc_tim_vec.transpose(0, 1))
                )
                seq_rel = torch.mm(att_seq, user_loc_tim_vec).unsqueeze(1)
                h2 = Variable(
                    torch.zeros(1, 1, self.hidden_units)
                ).cuda()  # [1, embedding_dim]
                out2, h2 = self.rel_rnn(seq_rel, h2)
                seq_W = Variable(
                    torch.FloatTensor(sequence_length - 1, sequence_len)
                ).cuda()
                nn.init.xavier_uniform(seq_W)
                hidden_sequence = torch.mm(seq_W, out2.squeeze())
                session_list.append(hidden_sequence.unsqueeze(0))
            sessions_represent = torch.cat(session_list, dim=0).transpose(
                0, 1
            )  # [sequence_length, len(session), embedding_dim]
            current_session_represent = current_session_represent.unsqueeze(
                2
            )  # [sequence_length, embedding_dim , 1]
            sims = F.softmax(
                sessions_represent.bmm(current_session_represent).squeeze(2), dim=1
            ).unsqueeze(
                1
            )  ##==> sequence_length * 1 * 1
            out_y_current = sims.bmm(sessions_represent).squeeze(
                1
            )  # [sequence_length* embedding_dim]
            ######################################################################################
            out_y_current_padd = Variable(
                torch.FloatTensor(
                    sequence_size - sequence_length + 1, self.emb_size
                ).zero_(),
                requires_grad=False,
